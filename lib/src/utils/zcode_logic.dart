var dagbr = {
  "yin": "ᡬᡬᡧ ", "in": "ᡬᡬᡧ ", "jin": "ᡬᡬᡧ ",
  "i": "ᡥᡫ ",
  "on": "ᡭᡧ ", "vn": "ᡭᡧ ",
  "o": "ᡳ ", "no": "ᡳ ",
  "yen": "ᡬᡬᡪᡧ ",
  "yer": "ᡬᡬᡪᢝ ",
  "iyen": "ᡬᡬᡪᡧ ", "iyan": "ᡬᡬᡪᡧ ", "eyen": "ᡬᡬᡪᡧ ", "eyan": "ᡬᡬᡪᡧ ",
  "iyar": "ᡬᡬᡪᢝ ", "iyer": "ᡬᡬᡪᢝ ", "eyar": "ᡬᡬᡪᢝ ", "eyer": "ᡬᡬᡪᢝ ",
  "yi": "ᡬᡫ ",
  "vd": "ᡭᡭᡧ ", "od": "ᡭᡭᡧ ",
  "vv": "ᡭᡳ ", "uu": "ᡭᡳ ", "oo": "ᡭᡳ ",
  "kino": "ᢤᡬᡱᡭ ",
  "radio": "ᢞᡪᢙᡬᡭ ",
  "america": "ᡥᡪᢎᢟᢞᡬᢤᡪᡨ ",
  "europe": "ᡥᢟᡭᡬᢞᡭᡶᡪᡨ ",
  "su": "ᢔᡭᡦ ",
  "ch": "ᡭᡭ ", "chi": "ᡭᡭᡫ ",
  "cino": "ᢚᡬᡱᡳᡨ ",
  "a": "ᡧ ", "e": "ᡨ ", "n": "ᡪ ", "d": "ᡭᡪ ",
  "kod": "ᢥᡭᢙᡦ ",
  "tatar": "ᢘᡪᢘᡪᢝ ",
  "buu": "ᡴᡭᡳ ", "boo": "ᡴᡭᡳ ",
  "mh": "ᢌᡭᡪᢊᡱᡱᡭᢐ ᢋᡭᡬᢎᡭᡧ ",
  "mm": "ᢌᡭᡪᢊᡱᡱᡭᢐ ᢌᡭᢞᡫ ",
  "hb": "ᢋᡭᡬᢑᡭᡧ ᡴᡭᡬᡬᢝ ",
  "um": "ᡥᡭᡬᡴᡭᢝ ᢌᡭᡪᢊᡱᡱᡭᢐ ",
  "unicode": "ᢜᡭᡬᡱᡬᢥᡭᢙᡦ ",

  //
  // databases["hebei"] = "ᡥᢨᢟᡳᢟᡫ";
  // databases["sh"] = "ᢗ";
  // databases["shi"] = "ᢗᡫ";
  // databases["zh"] = "ᢨ";
  // databases["zhi"] = "ᢨᡫ";
  // databases["english"] = "ᡥᡪᡪᢊᢊᢑᡫ";
  // databases["england"] = "ᡥᡪᡪᢊᢊᢑᡫ";
  // databases["tere"] = "ᢘᡪᢞᡧ";
};

var aArr = ["ᡥᡧ", "", "", "ᡥᡪ", "ᡪᡪ", "", "ᡪ", "ᡪᡪ", "", "ᡧ", "ᡪᡨ", "ᡨ"];
var eArr = ["ᡥᡨ", "", "", "ᡥ", "ᡪᡪ", "", "ᡪ", "ᡪᡪ", "", "ᡪᡨ", "ᡧ", "ᡨ"];
var iArr = ["ᡫ", "", "", "ᡥᡬ", "", "", "ᡬ", "ᡬᡬ", "ᡪᡬ", "ᡫ", "ᡬᡦ", ""];
var oArr = ["ᡥᡭ", "ᡥᡭ", "", "ᡥᡭ", "", "", "ᡭ", "ᡪᡭ", "", "ᡳ", "ᡭ", ""];
var uArr = ["ᡥᡭᡦ", "", "", "ᡥᡭᡬ", "", "", "ᡭ", "ᡭᡬ", "ᡪᡭᡬ", "ᡳ", "ᡭ", "ᡭᡦ"];
var nArr = ["ᡯ", "", "", "ᡯ", "", "", "ᡪ", "ᡱ", "", "ᡧ", "ᡰ", ""];
var bArr = ["ᡳ", "", "", "ᡳ", "ᡴ", "", "ᡳ", "ᡴ", "", "ᡲ", "", ""];
var pArr = ["ᡶ", "", "", "ᡶ", "ᡷ", "", "ᡶ", "ᡷ", "", "ᡵ", "", ""];
var hArr = ["ᡸ", "", "", "ᡸ", "ᢊ", "ᢋ", "ᡪᡪ", "ᢊ", "ᢋ", "ᢇ", "ᢇ", ""];
var gArr = ["ᢈ", "", "ᡪᡪ", "ᢈ", "ᢊ", "ᢋ", "ᡱᡱ", "ᢊ", "ᢋ", "ᢇ", "ᢉ", "ᡬᡨ"];
var mArr = ["ᢌ", "", "", "ᢌ", "", "", "ᢎ", "", "", "ᢍ", "", ""];
var lArr = ["ᢏ", "", "", "ᢏ", "", "", "ᢑ", "", "", "ᢐ", "", ""];
var sArr = ["ᢔ", "", "", "ᢔ", "", "", "ᢔ", "", "", "ᢓ", "", ""];
var xArr = ["ᢗ", "", "", "ᢗ", "", "", "ᢗ", "", "", "ᢖ", "", ""];
var tArr = ["ᢘ", "", "", "ᢘ", "", "", "ᢙ", "ᢘ", "", "ᢘᡦ", "", ""];
var dArr = ["ᢙ", "", "", "ᢘ", "", "", "ᡭᡪ", "ᢙ", "", "ᡭᡧ", "ᢙᡦ", ""];
var cArr = ["ᢚ", "", "", "ᢚ", "", "", "ᢚ", "", "", "ᢚᡦ", "", ""];
var jArr = ["ᡬ", "", "", "ᡬ", "", "", "ᢛ", "", "", "ᢛᡦ", "", ""];
var yArr = ["ᢜ", "", "", "ᢜ", "", "", "ᢜ", "", "", "ᡫ", "", ""];
var rArr = ["ᢞ", "", "", "ᢞ", "", "", "ᢞ", "", "", "ᢝ", "", ""];
var wArr = ["ᢟ", "", "", "ᢟ", "", "", "ᢟ", "", "", "ᢟᡦ", "", ""];
var fArr = ["ᢡ", "", "", "ᢡ", "ᢢ", "", "ᢡ", "ᢢ", "", "ᢠ", "", ""];
var kArr = ["ᢤ", "", "", "ᢤ", "ᢥ", "", "ᢤ", "ᢥ", "", "ᢣ", "", ""];
var qArr = ["ᢚ", "", "", "ᢚ", "", "", "ᢚ", "", "", "ᢚᡦ", "", ""];
var zArr = ["ᢧ", "", "", "ᢧ", "", "", "ᢧ", "", "", "ᢧᡦ", "", ""];
var vArr = ["ᡥᡭ", "ᡥᡭ", "", "ᡥᡭ", "", "", "ᡭ", "ᡪᡭ", "", "ᡳ", "ᡭ", ""];
var AArr = ["ᡥᡧ", "", "", "ᡪᡪ", "", "", "ᡪᡪ", "", "", "ᡧ", "", ""];
var EArr = ["ᢟ", "", "", "ᢟ", "", "", "ᢟ", "", "", "ᢟᡦ", "", ""];
var IArr = ["ᡫ", "", "", "ᡥᡬ", "", "", "ᡬ", "ᡬᡬ", "ᡪᡬ", "ᡫ", "ᡬᡦ", ""];
var OArr = ["ᡥᡭ", "ᡥᡭ", "", "ᡥᡭ", "", "", "ᡭ", "ᡪᡭ", "", "ᡭ", "ᡭ", ""];
var UArr = ["ᡥᡭᡦ", "", "", "ᡥᡭᡬ", "", "", "ᡭᡬ", "", "", "ᡭᡦ", "", ""];
var NArr = ["ᡯ", "", "", "ᡯ", "", "", "ᡱ", "ᡪ", "", "ᡧ", "ᡰ", ""];
var BArr = ["ᡳ", "", "", "ᡳ", "ᡴ", "", "ᡳ", "ᡴ", "", "ᡲ", "", ""];
var PArr = ["ᡶ", "", "", "ᡶ", "ᡷ", "", "ᡶ", "ᡷ", "", "ᡵ", "", ""];
var HArr = ["ᡸ", "", "", "ᢊ", "ᡸ", "ᢋ", "ᡪᡪ", "ᢊ", "ᢋ", "ᢇ", "ᢇ", ""];
var GArr = ["ᢈ", "", "ᡪᡪ", "ᢊ", "ᢈ", "ᢋ", "ᡱᡱ", "ᢊ", "ᢋ", "ᢇ", "ᢉ", "ᡬᡨ"];
var MArr = ["ᢌ", "", "", "ᢌ", "", "", "ᢎ", "", "", "ᢍ", "", ""];
var LArr = ["ᢏᢨ", "", "", "ᢏᢨ", "", "", "ᢑᢨ", "", "", "ᢑᢨᡦ", "", ""];
var SArr = ["ᢔ", "", "", "ᢔ", "", "", "ᢔ", "", "", "ᢓ", "", ""];
var XArr = ["ᢗ", "", "", "ᢗ", "", "", "ᢗ", "", "", "ᢖ", "", ""];
var TArr = ["ᢙ", "", "", "ᢙ", "", "", "ᢙ", "ᢘ", "", "ᢙᡦ", "", ""];
var DArr = ["ᢘ", "", "", "ᢙ", "", "", "ᢘ", "", "", "ᢘᡦ", "", ""];
var CArr = ["ᢦ", "", "", "ᢦ", "", "", "ᢦ", "", "", "ᢦᡦ", "", ""];
var JArr = ["ᡬ", "", "", "ᡬ", "", "", "ᢛ", "", "", "ᢛᡦ", "", ""];
var YArr = ["ᢜ", "", "", "ᢜ", "", "", "ᢜ", "", "", "ᡫ", "", ""];
var RArr = ["ᢪ", "", "", "ᢪ", "", "", "ᢪ", "", "", "ᢪᡦ", "", ""];
var WArr = ["ᢟ", "", "", "ᢟ", "", "", "ᢟ", "", "", "ᢟᡦ", "", ""];
var FArr = ["ᢡ", "", "", "ᢡ", "ᢢ", "", "ᢡ", "ᢢ", "", "ᢠ", "", ""];
var KArr = ["ᢤ", "", "", "ᢤ", "ᢥ", "", "ᢤ", "ᢥ", "", "ᢣ", "", ""];
var QArr = ["ᡮ", "", "", "ᡮ", "", "", "ᡮ", "", "", "ᡮ", "", ""];
var ZArr = ["ᢨ", "", "", "ᢨ", "", "", "ᢨ", "", "", "ᢨᡦ", "", ""];
var VArr = ["ᡭ", "ᡭ", "", "ᡭ", "", "", "ᡭ", "ᡪᡭ", "", "ᡳ", "ᡭ", ""];

class ZCode {
  var wordMap = new Map();

  ZCode() {
    wordMap["a"] = aArr;
    wordMap["e"] = eArr;
    wordMap["i"] = iArr;
    wordMap["o"] = oArr;
    wordMap["u"] = uArr;
    wordMap["n"] = nArr;
    wordMap["b"] = bArr;
    wordMap["p"] = pArr;
    wordMap["h"] = hArr;
    wordMap["g"] = gArr;
    wordMap["m"] = mArr;
    wordMap["l"] = lArr;
    wordMap["s"] = sArr;
    wordMap["x"] = xArr;
    wordMap["t"] = tArr;
    wordMap["d"] = dArr;
    wordMap["c"] = cArr;
    wordMap["j"] = jArr;
    wordMap["y"] = yArr;
    wordMap["r"] = rArr;
    wordMap["w"] = wArr;
    wordMap["f"] = fArr;
    wordMap["k"] = kArr;
    wordMap["q"] = qArr;
    wordMap["z"] = zArr;
    wordMap["v"] = vArr;

    wordMap["A"] = AArr;
    wordMap["E"] = EArr;
    wordMap["I"] = IArr;
    wordMap["O"] = OArr;
    wordMap["U"] = UArr;
    wordMap["N"] = NArr;
    wordMap["B"] = BArr;
    wordMap["P"] = PArr;
    wordMap["H"] = HArr;
    wordMap["G"] = GArr;
    wordMap["M"] = MArr;
    wordMap["L"] = LArr;
    wordMap["S"] = SArr;
    wordMap["X"] = XArr;
    wordMap["T"] = TArr;
    wordMap["D"] = DArr;
    wordMap["C"] = CArr;
    wordMap["J"] = JArr;
    wordMap["Y"] = YArr;
    wordMap["R"] = RArr;
    wordMap["W"] = WArr;
    wordMap["F"] = FArr;
    wordMap["K"] = KArr;
    wordMap["Q"] = QArr;
    wordMap["Z"] = ZArr;
    wordMap["V"] = VArr;
  }

  void PrintHello() {
    print("Hello ZCode");
  }

  Map FirstAndLast(String latin) {
    var ret = new Map();
    String firstStr = latin.substring(0, 1);
    String secondStr = latin.substring(1, 2);
    String lastStr = latin.substring(latin.length - 1, latin.length);
    String prelastStr = latin.substring(latin.length - 2, latin.length - 1);
    String resultFirst = "";
    var resultLast = [];
    if (wordMap.containsKey(firstStr)) {
      resultFirst = wordMap[firstStr][3];
      String x = firstStr;


      // 1. solving b,p,k,f  before o,u
      if (x == "b" || x == "p" || x == "k" || x == "f") {
        String y = secondStr;
        if (y == "o" || y == "u" || y == "v" || y == "U") {
          resultFirst = wordMap[x][4];
        }
      }
      // 2. solving h,g before e,i,w
      if (x == "h" || x == "g") {
        String y = secondStr;
        if (y == "e" || y == "i" || y == "w") {
          resultFirst = wordMap[x][4];
        }
      }

      // 3. solving h,g before u
      if (x == "h" || x == "g") {
        String y = secondStr;
        if (y == "u" || y == "U") {
          resultFirst = wordMap[x][5];
        }
      }

      //solving last letter;
      if (wordMap.containsKey(lastStr)) {
        String pTemp = wordMap[lastStr][9];
        if (pTemp.length > 0) {
          resultLast.add(pTemp);
        }
      }
      x = prelastStr;
      // 1. solving a after b,p,k,f    test haha  baba
      if (x == "b" || x == "p" || x == "k" || x == "f") {
        String y = lastStr;
        if (y == "a") {
          if (resultLast.length > 0) {
            resultLast.clear();
          }
          resultLast.add(wordMap[y][10]);
        }
      }
      // 2. solving e after h,g
      if (x == "h" || x == "g") {
        String y = lastStr;
        if (y == "e") {
          if (resultLast.length > 0) {
            resultLast.clear();
          }
          resultLast.add(wordMap[y][9]);
        }
      }

      // 3. solving i after b,p,k,f,h,g
      if (x == "b" ||
          x == "p" ||
          x == "k" ||
          x == "f" ||
          x == "h" ||
          x == "g") {
        String y = lastStr;
        if (y == "i") {
          if (resultLast.length > 0) {
            resultLast.clear();
          }
          resultLast.add(wordMap[y][10]);
        }
      }

      // 4. solving o after b,p,k,f
      if (x == "b" || x == "p" || x == "k" || x == "f") {
        String y = lastStr;
        if (y == "o" || y == "v") {
          if (resultLast.length > 0) {
            resultLast.clear();
          }
          resultLast.add(wordMap[y][10]);
        }
      }

      // 5. solving u after b,p,k,f,h,g
      if (x == "b" ||
          x == "p" ||
          x == "k" ||
          x == "f" ||
          x == "h" ||
          x == "g") {
        String y = lastStr;
        if (y == "u") {
          if (resultLast.length > 0) {
            resultLast.clear();
          }
          resultLast.add(wordMap[y][10]);
        }
      }

      // 6. solving g after i,e,u, U
      if (x == "i" || x == "e" || x == "u" || x == "U") {
        String y = lastStr;
        if (y == "g") {
          if (x == "i") {
            for (int j = 1; j < latin.length - 2; j++) {
              String z =
                  latin.substring(latin.length - 2 - j, latin.length - 1 - j);
              if (z == "a" || z == "o" || z == "v") {
              }
            }
            if (resultLast.length > 0) {
              resultLast.clear();
            }
            resultLast.add(wordMap[y][9]);
          } else {
            if (resultLast.length > 0) {
              resultLast.clear();
            }
            resultLast.add(wordMap[y][11]);
          }
        }
      }

      // 7. solving g after n
      if (x == "n") {
        String y = lastStr;
        if (y == "g") {
          if (resultLast.length > 0) {
            resultLast.clear();
          }
          resultLast.add(wordMap[y][11]);
        }
      }
      // 8. solving a, e after n, m, l, y, r
      if (x == "n" || x == "m" || x == "l" || x == "y" || x == "r") {
        String y = lastStr;
        if (y == "a" || y == "e") {
          if (resultLast.length > 0) {
            resultLast.clear();
          }
          resultLast.add(wordMap[y][11]);
        }
      }
      // 9. solving e after c, v, z, q, d
      if (x == "c" || x == "v" || x == "z" || x == "q" || x == "d") {
        String y = lastStr;
        if (y == "e") {
          if (resultLast.length > 0) {
            resultLast.clear();
          }
          resultLast.add(wordMap[y][10]);
        }
      }
      // 10. solving a, after h, g
      if (x == "h" || x == "g") {
        String y = lastStr;
        if (y == "a") {
          if (resultLast.length > 0) {
            resultLast.clear();
          }
          resultLast.add(wordMap[y][11]);
        }
      }
      // 11. solving a, after n, m, h, g, for isolated na, ma, ha, ga
      if (x == "n" || x == "m" || x == "h" || x == "g") {
        String y = lastStr;
        if (y == "a") {
          if (2 == latin.length) {
            if (resultLast.length > 0) {
              resultLast.clear();
            }
            resultLast.add(wordMap[y][9]);
          }
        }
      }
    }

    ret['first'] = resultFirst;
    ret['last'] = resultLast;

    return ret;
  }

  List<String> Middle(String str, bool isForML) {
    String resultMid = "";
    String nMinusTwo = "";
    String m = "";

    var maxLength = isForML ? str.length : str.length - 1;
    for (int i = 1; i < maxLength; i++) {
      String x = str.substring(i, i + 1);
      if (wordMap.containsKey(x)) {
        m = wordMap[x][6];

        //1. solving g debisger after a or o
        String yy = str.substring(i - 1, i);
        if (yy == "a" || yy == "o" || yy == "v") {
          if (x == "g") {
            m = wordMap[x][2];
          }
        }

        //2. solving g not after a or o
        if (yy != "a" && yy != "o" && yy != "v") {
          if (x == "g") {
            m = wordMap[x][7];
          }
        }

        //3. solving N,T before egshig    a,e,i,o,u, U, A, E,
        if (x == "N" || x == "T" || x == "D") {
          m = wordMap[x][7];
        }

        //4. solving u for second position,    test bumbur  bum
        if (x == "u" && i == 1) {
          m = wordMap[x][7];
        }

        //5. solving i after a, e, o
        if (x == "i") {
          String y = str.substring(i - 1, i);
          if (y == "a" || y == "e" || y == "o" || y == "v") {
            m = wordMap[x][7];
          }
        }

        if (i != str.length - 1) {
          // 6. solving b,p,k,f  before o,u
          if (x == "b" || x == "p" || x == "k" || x == "f") {
            String y = str.substring(i + 1, i + 2);
            if (y == "o" || y == "u" || y == "v" || y == "U") {
              m = wordMap[x][7];
            }
          }

          //7. solving n,d before egshig    a,e,i,o,u, U, A, E,
          if (x == "n" || x == "d") {
            String y = str.substring(i + 1, i + 2);
            if (y == "a" ||
                y == "e" ||
                y == "i" ||
                y == "o" ||
                y == "v" ||
                y == "u" ||
                y == "w" ||
                y == "A" ||
                y == "E" ||
                y == "U") {
              m = wordMap[x][7];
            }
          }

          //8. solving g before a or o
          if (x == "g") {
            String y = str.substring(i + 1, i + 2);
            if (y == "a" || y == "o" || y == "v") {
              m = wordMap[x][6];
            }
          }

          //9. solving h, g before e, i, w
          if (x == "h" || x == "g") {
            String y = str.substring(i + 1, i + 2);
            if (y == "e" || y == "i" || y == "w") {
              m = wordMap[x][7];
            }
          }

          //10. solving h, g before u
          if (x == "h" || x == "g") {
            String y = str.substring(i + 1, i + 2);
            if (y == "u") {
              m = wordMap[x][8];
            }
          }

          //11. solving g after Positive or Negative i traceback and  not before a,e,i, o, u      test yabogsan uggugsen jarlig jerlig baigal
          if (x == "g") {
            // get the character BEFORE current character
            String y = str.substring(i - 1, i);

            // get the character AFTER current character
            String w = str.substring(i + 1, i + 2);
            if (y == "i") {
              if (w != "a" &&
                  w != "e" &&
                  w != "i" &&
                  w != "o" &&
                  w != "v" &&
                  w != "u") {
                    
                // 2021.08.09; increase max limit for j by 1 
                // previously is j <= i, changed to j <= i + 1
                for (int j = 1; j <= (i + 1); j++) {
                  String z = str.substring(i + 1 - j, i + 2 - j);
                  if (z == "a" || z == "o" || z == "v" || z == "A") {
                    m = wordMap[x][2];
                  }
                }
              }
            }
          }

          // The following logic is not used when generating word for ML
          if (!isForML) {
            //12. h, g before a , for second last letter
            if (x == "h" || x == "g") {
              String y = str.substring(i + 1, i + 2);
              if (y == "a") {
                if (i == str.length - 2) {
                  m = wordMap[x][10];
                }
              }
            }

            //13. n before a, e, for second last letter      test yabona
            if (x == "n") {
              String y = str.substring(i + 1, i + 2);
              if (y == "a" || y == "e") {
                if (i == str.length - 2) {
                  m = wordMap[x][10];
                }
              }
            }

            //14. m, L, y, r before a, e
            if (x == "m" || x == "l" || x == "y" || x == "r") {
              String y = str.substring(i + 1, i + 2);
              if (y == "a" || y == "e") {
                if (i == str.length - 2) {
                  m = wordMap[x][9];
                }
              }
            }
          }
        }

        nMinusTwo = resultMid;
        resultMid = resultMid + m;
      }
    }
    return [resultMid, nMinusTwo];
  }

  List GetEndsClass(String word) {
    var newWords = [];
    //0:Unknown   1:ne 2:me 3:le 4:ye 5:re 6:wa  7:o-3,u-3
    int iRet = 0;

    if (word == oArr[0]) {
      //o
      newWords.add(eArr[3] + uArr[9]);
      newWords.add(oArr[9]);
      newWords.add(oArr[6]);
    } else if (word == eArr[3] + uArr[11]) {
      //u
      newWords.add(eArr[3] + uArr[9]);
      newWords.add(uArr[3]);
    }

    // if lastcharacter = gArr[9],  lastcharacter +=gArr[11]
    else if (word.length > 2) {
      String scLast = word.substring(word.length - 1, word.length);
      String sPreLast = word.substring(word.length - 2, word.length - 1);
      String subWordPre = word.substring(0, word.length - 2);
      if (scLast == aArr[11]) {
        if (sPreLast == nArr[10]) {
          //na
          subWordPre += nArr[7] + aArr[9];
          iRet = 1;
        } else if (sPreLast == mArr[9]) {
          //ma
          subWordPre += mArr[6] + aArr[9];
          iRet = 2;
        } else if (sPreLast == lArr[9]) {
          //la
          subWordPre += lArr[6] + aArr[9];
          iRet = 3;
        }
        else if (sPreLast == yArr[9]) {
          //ya
          subWordPre += yArr[6] + aArr[9];
          iRet = 4;
        }
        else if (sPreLast == rArr[9]) {
          //ra
          subWordPre += rArr[6] + aArr[9];
          iRet = 5;
        }
      }
      else if (sPreLast == wArr[6] && scLast == aArr[9]) {
        //wa
        subWordPre += oArr[9] + aArr[11];
        iRet = 6;
      }
      // ending g debisger
      else if (scLast == gArr[9]) {
        subWordPre += iArr[6] + gArr[11];
        iRet = 7;
      }
      // ending d debisger
      else if (sPreLast == oArr[6] && scLast == aArr[9]) {
        subWordPre += dArr[10];
        iRet = 8;
      }
      if (iRet != 0) {
        newWords.add(subWordPre);
      }
    }

    return newWords;
  }

  List Excute(String latin) {
    var result = [];
    int wordLength = latin.length;
    String resultFirst = "";
    var arrMid = [];
    var resultLast = [];
    if (wordLength == 1 && wordMap.containsKey(latin)) {
      resultFirst = wordMap[latin][0];
    } else if (wordLength == 2) {
      var firstlast = FirstAndLast(latin);
      resultFirst = firstlast['first'];
      resultLast = firstlast['last'];
      arrMid = Middle(latin, false);
    } else {
      var firstlast = FirstAndLast(latin);
      resultFirst = firstlast['first'];
      resultLast = firstlast['last'];
      arrMid = Middle(latin, false);
    }

    if (resultLast.length > 0) {
      for (int i = 0; i < resultLast.length; i++) {
        result.add(resultFirst + arrMid[0] + resultLast[i]);
      }
    } else {
      result.add(resultFirst);
    }

    return result;
  }

  // Returns mongolian word without the tail (ending)
  List ExcuteForML(String latin) {
    var result = [];
    int wordLength = latin.length;
    String resultFirst = "";
    var arrMid = [];
    var resultLast = [];

    /// 1st logic (AI)
    if (wordLength == 1 && wordMap.containsKey(latin)) {
      resultFirst = wordMap[latin][0];
    } else {
      var firstlast = FirstAndLast(latin);
      resultFirst = firstlast['first'];
      resultLast = firstlast['last'];
      arrMid = Middle(latin, true);
    }

    if (resultLast.length > 0) {
      for (int i = 0; i < resultLast.length; i++) {
        // var lastChar = latin.substring(latin.length - 1);
        // var secondLastChar =
        //     latin.substring(latin.length - 2, latin.length - 1);
        // var midOfLastChar = wordMap[lastChar][6];
        // var midOfSecondLastChar = wordMap[secondLastChar][6];

        if (latin.length > 2) {
          result.add(resultFirst + arrMid[0]);
        } else {
          result.add(resultFirst + arrMid[0]);
        }
      }
    } else {
      result.add(resultFirst);
    }

    return result;
  }

  //2021.3.9
  List ExcuteEx(String latin) {
    var result = [];
    var resWords = Excute(latin) + ExcuteForML(latin);
    for (int i = 0; i < resWords.length; i++) {
      if (resWords[i].length > 0) {
        result.add(resWords[i]);

        //ne me le ye re wa
        var newWord = GetEndsClass(resWords[i]);
        if (newWord.length > 0) {
          result.addAll(newWord);
        }
      }
    }

    return result;
  }
}
