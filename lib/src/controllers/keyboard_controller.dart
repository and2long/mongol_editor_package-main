...
import '../utils/zcode_logic.dart';
import '../ml/mongol_ml_autocomplete_optimized.dart';
...
class KeyboardController extends GetxController {
  ...
  var zcode = new ZCode();
  // Use optimized ML autocomplete with caching and debouncing
  var mlAutocomplete = MongolMLAutocomplete(
    blockSize: 20,
    numberOfSampleWords: 10,
    maxLengthOfWord: 20,
  );
  ...
  void onInit() {
    ...
    // Try to initialize ML, but continue if it fails
    mlAutocomplete
        .initialize()
        .then((_) {
          mlEnabled = true;
          debugPrint('ML Autocomplete enabled');
        })
        .catchError((e) {
          mlEnabled = false;
          debugPrint('ML Autocomplete disabled (missing assets): $e');
        });
  }
  ...
  void onClose() {
    super.onClose();
    // DON'T dispose controllers for singleton - just reset state
    // textEditingController.dispose(); // Keep controller alive
    // scrollController.dispose(); // Keep controller alive
    // mlAutocomplete.dispose(); // Keep ML resources loaded
  ...
  /// disable [addExtraSpace] when adding emojis, because it looks weird.
  void enterAction(String? v, {bool addExtraSpace = true}) async {
    if (v == null) {
      ...
      String prefixText = textEditingController.text.substring(
        0,
        cursorPosition,
      );

      textEditingController.text = prefixText + v + suffixText;
      ...
      if (newCursorPosition > textEditingController.text.trim().length - 1) {
        Timer(Duration(milliseconds: 100), () => scrollToEnd());
      }
    }

    latin.value = '';
    cands.clear();
    var suggestions = await _getSuggestionsFromML([
  ...
  void deleteOne() async {
    ...
    final int currentPosition = textSelection.start;
    final textToBeDeleted = text.substring(0, currentPosition);
    final textDeleted = textToBeDeleted.characters
        .skipLast(1)
        .toString(); // Remove character before cursor

    // Merge text after delete with remainder after cursor
    textEditingController.text = textDeleted + text.substring(currentPosition);

    final deletedCharacterLength = textToBeDeleted.length - textDeleted.length;
    textEditingController.selection = textSelection.copyWith(
      baseOffset: currentPosition - deletedCharacterLength,
      extentOffset: currentPosition - deletedCharacterLength,
    );

    debugPrint('deleted $deletedCharacterLength chars');
  ...
  suggestWords() async {
    ...
    if (latin.isNotEmpty) {
      ...
      List<String> wordWithoutTail = words.sublist(
        words.length - 1,
        words.length,
      );
  ...
  Future<List<String>> _getSuggestionsFromML(
    ...
    // Return empty list if ML is not enabled
    if (!mlEnabled) {
      return [];
    }
    ...
    try {
      ...
      final cleanedResult = result
          ...
          .toList();
      debugPrint('ML result: $cleanedResult');

      return cleanedResult;
    } catch (e) {
      debugPrint('ML suggestion error: $e');
      return [];
    }
  }
